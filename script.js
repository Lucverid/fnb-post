...
tr.innerHTML =   <td>${p.name || "-"}</td>   <td>${p.type === "menu" ? "Menu" : "Bahan Baku"}</td>   <td>${p.category || "-"}</td>   <td>${p.type === "menu" ? formatCurrency(p.price || 0) : "-"}</td>   <td>${p.type === "bahan_baku" ? p.stock || 0 : "-"}</td>   <td>${st.label}</td>   <td>   <button data-act="edit" data-id="${p.id}">Edit</button>   <button data-act="del" data-id="${p.id}">Hapus</button>   </td>  ;
...
list.forEach((m) => {
const tr = document.createElement("tr");
tr.innerHTML =   <td>${m.name || "-"}</td>   <td>${formatCurrency(m.price || 0)}</td>   <td><button data-id="${m.id}">Tambah</button></td>  ;
...
const tr = document.createElement("tr");
tr.innerHTML =   <td>${it.name}</td>   <td>${it.qty}</td>   <td>${formatCurrency(it.subtotal)}</td>   <td><button data-idx="${idx}">x</button></td>  ;
...
.map(
(it) =>    <div style="display:flex;justify-content:space-between;font-size:12px;">   <span>${it.name} x${it.qty}</span>   <span>${formatCurrency(it.subtotal)}</span>   </div>
)
.join("");
printArea.innerHTML =    <div style="text-align:center;font-weight:700;margin-bottom:4px;">F&B POS</div>   <div style="font-size:11px;margin-bottom:6px;">   ${waktu}<br/>Kasir: ${kasir}   </div>   <hr/>   ${itemsHtml || '<div style="font-size:11px;">(Tidak ada item)</div>'}   <hr/>   <div style="font-size:11px;">   <div style="display:flex;justify-content:space-between;">   <span>Total</span><span>${formatCurrency(saleDoc.total)}</span>   </div>   <div style="display:flex;justify-content:space-between;">   <span>Bayar</span><span>${formatCurrency(saleDoc.pay)}</span>   </div>   <div style="display:flex;justify-content:space-between;">   <span>Kembalian</span><span>${formatCurrency(saleDoc.change)}</span>   </div>   </div>;
}
...
const tr = document.createElement("tr");
tr.innerHTML =   <td>${name}</td>   <td>${data.qty}</td>   <td>${formatCurrency(data.total)}</td>  ;
topMenuTable.appendChild(tr);
});
}
...
bahan.forEach((p) => {
const tr = document.createElement("tr");
tr.innerHTML =   <td>${p.name}</td>   <td>${p.stock || 0} ${p.unit || ""}</td>   <td><input type="number" data-id="${p.id}" value="${p.stock || 0}"></td>   <td><span data-id="${p.id}-diff">0</span></td>   <td><button data-id="${p.id}">Simpan</button></td>  ;
